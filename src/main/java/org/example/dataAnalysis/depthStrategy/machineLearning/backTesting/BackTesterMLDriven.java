// Corrected and Updated BackTesterMLDriven.java with Confidence Threshold, Target Profit and Stop Loss
...
public class BackTesterMLDriven {
    ...
    private static final double TARGET_MOVE_THRESHOLD = 0.002;
    private static final double MIN_CONFIDENCE_THRESHOLD = 0.6;
    private static final double TARGET_PROFIT_PERCENT = 0.004;
    private static final double STOP_LOSS_PERCENT = 0.002;
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int symbolId : SYMBOL_IDS) {
            ...
            for (int i = 0; i < compressedTicks.size() - LOOKAHEAD_TICKS; i++) {
                ...
                String label;
                if (move > TARGET_MOVE_THRESHOLD) {
                    label = "BUY";
                } else if (move < -TARGET_MOVE_THRESHOLD) {
                    label = "SELL";
                } else {
                    label = "HOLD";
                }
                ...
                if (featureList.size() < INITIAL_TRAINING_SIZE) {
                    continue;
                }
                ...
                if (predictionResult.confidence < MIN_CONFIDENCE_THRESHOLD) {
                    continue;
                }
                ...
                if (position != null) {
                    ...
                    if (shouldExit) {
                        ...
                        System.out.printf("Trade: %s | Entry: %.2f @ %s | Exit: %.2f @ %s | PnL: %.2f\n",
                                position, entryPrice, FORMATTER.format(Instant.ofEpochSecond(entryTime)),
                                exitPrice, FORMATTER.format(Instant.ofEpochSecond(tick.getLastTradedTime())), pnl);
